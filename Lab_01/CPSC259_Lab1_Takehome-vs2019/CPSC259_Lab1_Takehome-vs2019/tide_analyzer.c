...
void run_analysis()
{
  ...
  for (i = 0; i < NUMBER_OF_READINGS; i++) {
      omega[i] = i * (double)SAMPLING_FREQUENCY / (double)NUMBER_OF_READINGS;
  }
  ...
  file_pointer = fopen(FILE_NAME, "r"); //(for PrairieLearn submission)
  ...
  fft(1, 17, readings, complex_component);
                                            ...
                                            */
  for (i = 0; i < NUMBER_OF_READINGS; i++) {
      readings[i] = pow((pow(readings[i],2) + pow(complex_component[i],2)),0.5);
                                            }
  ...
  file_pointer = fopen(RESULT_FILE_NAME, "w"); //(for PrairieLearn submission)
...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
  ...
  while (fgets(line_buffer, LINESIZE, pointer_to_data_file)) {
      ...
      values_per_line = sscanf(line_buffer, "%d %d %d %d %d %d", &extracted_values[0], &extracted_values[1],
          &extracted_values[2], &extracted_values[3], &extracted_values[4], &extracted_values[5]); //(for PrairieLearn submission)


      /* Copies the extracted integers to our data array.  Use a for loop for each
      for each of the cells in the local array, and add the value
